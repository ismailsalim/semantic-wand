...
class Pipeline:
    ...
    def __call__(self, img):
        ...
        self.alpha_feedback(alpha, size, 0)

        return self.results

    
    def alpha_feedback(self, alpha, size, iteration):
        if iteration < self.iterations:
            trimap = self.trimap_stage.process_alpha(alpha, size)
            self.results['trimaps'].append(trimap)

            fg, alpha, matte = self.to_refinement_stage(trimap, self.img)
            self.results['foregrounds'].append(fg*255)
            self.results['alphas'].append(alpha*255)
            self.results['mattes'].append(matte*255)

            self.alpha_feedback(alpha, size, iteration+1)
